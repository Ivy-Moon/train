    ...
    * */
    ...
    ulBox.addEventListener("touchend",function(e){
        ...
        if(distanceY+currentY > maxPotinstion){
            ...
            //设置位置
            ulBox.style.webkitTransform="translateY("+currentY+"px)";
        }
            ...
            //设置位置
            ulBox.style.webkitTransform="translateY("+currentY+"px)";
            ulBox.style.transform="translateY("+currentY+"px)";
        }
        else{
            currentY+=distanceY;
        }
    ...
    itcast.tap(ulBox,function(e){
        ...
        if((-index*50) < minPosition){
            ...
            ulBox.style.transition="all .2s";
            //设置位置
            ulBox.style.webkitTransform="translateY("+(minPosition)+"px)";
            ulBox.style.transform="translateY("+(minPosition)+"px)";
        }
        else{
            //开启过渡
            ulBox.style.webkitTransition="all .2s";
            ulBox.style.transition="all .2s";
            //设置位置
            ulBox.style.webkitTransform="translateY("+(-index*50)+"px)";
            ulBox.style.transform="translateY("+(-index*50)+"px)";
            //重置currentY
            currentY=-index*50;
        }
    });
